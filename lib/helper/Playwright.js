...
class Playwright extends Helper {
  ...
  async _after() {
    ...
    if (this.isRemoteBrowser) {
      this.remoteSessions.forEach(session => session.close());
    }
        ...
        if (this.isElectron) {
          ...
          try {
            if (this.isCDPConnection) {
              const browser = await playwright[this.options.browser].connectOverCDP(this.playwrightOptions);
              this.remoteSessions.push(browser);
              browserContext = await browser.newContext(config);
              this.browser._contexts.add(browserContext);
            } else {
              const browser = await playwright[this.options.browser].connect(this.playwrightOptions);
              this.remoteSessions.push(browser);
              browserContext = await browser.newContext(config);
              this.browser._contexts.add(browserContext);
            }
